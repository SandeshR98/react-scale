...
export default function App() {
	...
	const [uiWorkerEnabled, setUiWorkerEnabled] = useState(
		INITIAL_STATE.workerEnabled,
	);

	// Lifted out of ProductList so opening a modal never re-renders the 5K list.
	const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
	const handleProductClick = useCallback(
		(p: Product) => setSelectedProduct(p),
		[],
	);
	...
	return (
		<div className='flex h-screen overflow-hidden bg-background text-foreground'>
			<div className='flex flex-col flex-1 min-w-0 overflow-hidden'>
				...
				>
					{state.isLoading ? (
						<div className='overflow-auto h-full'>{loadingFallback}</div>
					) : (
						<ProductList
